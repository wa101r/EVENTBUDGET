...
<template>
  <div v-if="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
    <div
      class="bg-white w-full max-w-lg rounded-3xl shadow-2xl flex flex-col max-h-[90vh] animate-scale-up overflow-hidden">
      <header class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        ...
        <button @click="$emit('close')"
          class="w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center text-gray-500 transition-colors">✕</button>
      </header>

      <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar">
        <UiInput :model-value="form.title" @update:model-value="v => update('title', v)"
          :label="t.label_activity_name" />

        <div class="grid grid-cols-2 gap-5">
          <UiInput :model-value="form.start_time" @update:model-value="v => update('start_time', v)"
            :label="t.label_start_time" type="time" />
          <UiInput :model-value="form.end_time" @update:model-value="v => update('end_time', v)"
            :label="t.label_end_time" type="time" />
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-5 gap-3">
            <button v-for="icon in availableIcons" :key="icon" @click="update('icon', icon)" :class="[
        ...
        <div class="space-y-4">
          <UiInput :model-value="form.location" @update:model-value="v => update('location', v)"
            :label="t.label_location_timeline" />
          <UiTextarea :model-value="form.description" @update:model-value="v => update('description', v)"
            :label="t.label_desc_timeline" rows="3" />
        </div>
      </div>
      ...
      <footer class="px-8 py-5 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
        ...
        <UiButton variant="primary" @click="$emit('save')"
          class="!bg-[#F47A27] !text-white hover:!bg-[#d96b22] !border-none">
          {{ t.save }}
        </UiButton>
      </footer>
    </div>
