...
<template>
  <div v-if="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm transition-opacity">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 animate-scale-up">
      ...
      <div class="space-y-5">
        <UiInput
          ...
          @update:model-value="v => update('date', v)"
          :label="t.label_event_date"
          type="date"
        />
        <UiInput
          :model-value="form.title"
          @update:model-value="v => update('title', v)"
          :label="t.label_theme_title"
        />
      </div>

      <div class="grid grid-cols-2 gap-3 mt-8">
        <UiButton variant="secondary" @click="$emit('close')" class="justify-center">
          {{ t.cancel }}
        </UiButton>
        <UiButton variant="primary" @click="$emit('save')" class="justify-center">
          {{ t.save }}
        </UiButton>
      </div>
    </div>
